import os
os.environ["PATH"] += os.pathsep + 'D:/python/Graphviz2.38/bin/'
# 导入csv格式文件
import csv
import graphviz
import pydotplus
...
dot2png(dot_file_path="D:\\python\\Django-2.2.24\\milk\\tree\\treeone.dot", img_path="D:\\python\\Django-2.2.24\\milk\\static\img\\tree.png")
# 通过以上求解把模型建立好，然后将处理后的数据套入模型内，用于预测结果
text = ([[0, 1, 0, 1, 0, 1, 1, 0]])
prediction = clf.predict(text)
print("预测结果为")
print(str(prediction))

# 开头已经导入 accuracy_score 模块，套入模型进行计算
# 预测精度会在下方展示
print("预测准确率为:")
print(accuracy_score(baseY, clf.predict(baseX)))

#with open("treeone.dot") as f:
    #dot_graph = f.read()
